...
def TrainMLP(name_wind, name_day, cap, hidden_layers=[32], batch_size=64, num_epochs=500, weight_decay=1e-4, index_features=[0, 1, 2, 3, 4]):
    ...
    torch.save(model.state_dict(), name_file + 'mlp_model_uvspa.pth')
...
def Forecast1(file_weather, name_wind, longitude, latitude, a_input, b_input, a_output, b_output, today, hidden_layers=[32], name_features=['u100', 'v100', 'sp', 'tcw', 'a100'], l_hours=21, h_hours=51):
    ...
    df_weather = df_weather.loc[(df_weather['longitude'] == longitude) & (df_weather['latitude'] == latitude), ['time', 'u100', 'v100', 'sp', 'a100']]
    ...
    model = MLP(num_inputs, num_outputs, hidden_layers)
    model.load_state_dict(torch.load('data/MediateData/DataMatrix1_' + name_wind + '_' + 'mlp_model_uvspa.pth'))
...
def Forecast4(file_weather, name_wind, longitude, latitude, a_input, b_input, a_output, b_output, today, hidden_layers=[32], name_features=['u100', 'v100', 'sp', 'tcw', 'a100'], l_hours=93, h_hours=122):
    ...
    model = MLP(num_inputs, num_outputs, hidden_layers)
    model.load_state_dict(torch.load('data/MediateData/DataMatrix4_' + name_wind + '_' + 'mlp_model_uvspa.pth'))
...
def main():
    today_str = '20240803'
    file_weather = 'data/Input4Forecast/2024080300.csv'

    print('*************************Wind Farm D*************************')
    longitude = 111.5
    latitude = 21.4
    a_input=[51, 51, 4700, 940]
    b_input=[0.57, 0.57, -20.94, 0]
    a_output=410
    b_output=0.01
    cap = 399.9 / a_output + b_output
    hidden_layers=[128, 128, 128]
    TrainMLP('D', '1', cap, hidden_layers=hidden_layers, num_epochs=500, weight_decay=1e-3, index_features=[0, 1, 2, 4])
    output1 = Forecast1(file_weather, 'D', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
    TrainMLP('D', '4', cap, hidden_layers=hidden_layers, num_epochs=130, weight_decay=1e-4, index_features=[0, 1, 2, 4])
    output4 = Forecast4(file_weather, 'D', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
    ...
    hidden_layers=[128, 128, 128]
    TrainMLP('E', '1', cap, hidden_layers=hidden_layers, num_epochs=150, weight_decay=1e-3, index_features=[0, 1, 2, 4])
    output1 = Forecast1(file_weather, 'E', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
    TrainMLP('E', '4', cap, hidden_layers=hidden_layers, num_epochs=100, weight_decay=1e-4, index_features=[0, 1, 2, 4])
    output4 = Forecast4(file_weather, 'E', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
    ...
    hidden_layers=[128, 128, 128]
    TrainMLP('F', '1', cap, hidden_layers=hidden_layers, num_epochs=500, weight_decay=1e-3, index_features=[0, 1, 2, 4])
    output1 = Forecast1(file_weather, 'F', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
    TrainMLP('F', '4', cap, hidden_layers=hidden_layers, num_epochs=130, weight_decay=1e-4, index_features=[0, 1, 2, 4])
    output4 = Forecast4(file_weather, 'F', longitude, latitude, a_input, b_input, a_output, b_output, pd.to_datetime(today_str), hidden_layers=hidden_layers, name_features=['u100', 'v100', 'sp', 'a100'])
