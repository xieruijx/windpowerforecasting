...
def ProcessWeather(name, longitude, latitude):
    ...
    for index_df, _ in df.iterrows():
        ...
        input_filename = 'data/HistoricalWeatherForecastData/Type2/' + index.strftime('%Y%m%d') + '06.csv'
        if os.path.exists(input_filename):
            df_input = pd.read_csv(input_filename)
            if not 'tcw' in df_input.columns:
                index1y = index + pd.Timedelta(days=365)
                df_input_tcw = pd.read_csv('data/HistoricalWeatherForecastData/Type2/' + index1y.strftime('%Y%m%d') + '06.csv')
            ...
            df_output.to_csv(foldername + index.strftime('%Y%m%d') + '14.csv')
